...
> 目标：将 Claude Code CLI 容器化，自带代理（Mihomo + metacubexd Web 面板）和多 LLM 供应商切换能力，对外暴露一个 `dcc` 命令，使用体验等同原生 `claude`。
...
- 宿主机敲 `dcc` 命令，体感和敲 `claude` 完全一致。

### 不在范围
- 不容器化原版 Clash Verge GUI（用 Mihomo CLI + Web 面板替代，详见 §3）。
- 不容器化原版 cc-switch GUI（用 `dcc-use` CLI 脚本替代）。
...
│ 宿主机                                                   │
│                                                          │
│   dcc / dcc-use 命令 (shell wrapper)                       │
│        │                                                 │
│        ├─ dcc up      → docker compose up -d mihomo      │
│        ├─ dcc <args>  → docker compose run --rm cc claude│
│        └─ dcc-use X   → 直接改 ~/.docker-cc/claude/...   │
│                                                          │
│   ~/.docker-cc/                  ← 持久化目录            │
│     ├─ mihomo/  (config.yaml)                            │
...
| 订阅获取 | entrypoint 内 `curl -fsSL "$CLASH_SUB_URL"` 直接拉 | 订阅 URL 必须返回 **Clash/Mihomo 兼容的 yaml**；V2Ray/Trojan 等其他格式需用户先用 [subconverter](https://github.com/tindy2013/subconverter) 或 [sub.dler.io](https://sub.dler.io) 转成 Clash 格式后再传给 `dcc up <url>` |
...
| 供应商切换 | 自写 `dcc-use` shell 脚本 | 修改 `~/.claude/settings.json` 的 env 段；支持 API key 与 Anthropic OAuth 两种模式 |
| OAuth 登录 | Claude Code 内置 `/login`（device code flow） | 凭据落 `~/.claude/.credentials.json`，由卷挂载持久化 |
| 主入口 | 自写 `dcc` shell 脚本（宿主侧）| docker compose 的薄封装 |
...
`mirror.ghproxy.com` 经常抽风。如果某天 `dcc upgrade` 失败：

```bash
dcc probe                  # 重新探测，写入 .env
dcc upgrade                # 再试
```

`dcc probe` 与 `install.sh` 用的是同一组备用源 + 同一探测逻辑。
...
services:
  mihomo:
    ...
    build:                                  # dcc upgrade 走 docker compose build
      context: .
      args:
...
关键点：
- `profiles: ["cli"]` 让 `dcc` 服务默认不被 `compose up` 启动，只能通过 `compose run --rm cc` 调用。
- `${HOST_PWD}` 由 `dcc` 包装脚本在 `cd` 之前 export，保留用户调用时的原始工作目录。如果直接用 `docker compose run`（不通过 dcc 脚本），fallback 到 `${PWD}`（compose 命令的 pwd）。
...
### 7.1 `bin/dcc`（宿主端 wrapper）

放到 `~/bin/dcc`，软链 `/usr/local/bin/dcc`。
...
case "${1:-}" in
  up)
    ...
    if [ -n "${1:-}" ]; then
      ...
      [ -f .env ] && grep -q '^CLASH_SUB_URL=.' .env \
        || { echo "首次使用请指定订阅: dcc up <subscription-url>"; exit 1; }
  ...
  probe)
          ...
          # 用途：dcc upgrade 失败时手动救援，或主动切到更快的镜像源
  ...
  upgrade)
          ...
          case "${1:-claude}" in
            ...
            mihomo)
              echo "升级 mihomo 内核：修改 Dockerfile 的 ARG MIHOMO_VERSION，然后 dcc upgrade claude" ;;
            *)
              echo "用法: dcc upgrade [claude|mihomo|all]"; exit 1 ;;
          esac ;;
  update)
          echo "提示：dcc update 不会持久化。请使用 dcc upgrade（rebuild 镜像）。"
          exit 1 ;;
  *)
    # 默认：透传给 claude
...
- 首次调用 `ddcc <args>` 时，若 mihomo 未起，会自动 `up`。
- `dcc` 不带参数 → 交互 claude；`dcc -p "..."` → 一次性，与原生体验一致。

### 7.2 `bin/dcc-use`（LLM 供应商管理 + 切换）

对标原版 cc-switch GUI 的功能，提供完整的"管理 + 切换"能力，子命令模式：

```bash
# —— 查看 ——
dcc-use                              # 列出所有供应商，★ 标记当前激活（list 别名）
dcc-use list                         # 同上
...
dcc-use add <name> --mode=oauth      # 创建 OAuth 模式供应商（仅写入 _mode=oauth，无其他字段）
dcc-use add <name>                   # 缺参数则交互式补齐（API Key 用 -s 隐式输入）
dcc-use edit <name>                  # 用 $EDITOR 打开 ~/.docker-cc/providers/<name>.json
...
# —— 调试 ——
dcc-use test [<name>]                # 用 curl 探测 base_url 是否可达；不传 name 则测当前激活的
```
...
# 其余 cmd_list / cmd_current / cmd_show / cmd_edit / cmd_remove / cmd_test 略，详见 bin/dcc-use 源码
...
case "${1:-list}" in
  ...
  test)       shift; cmd_test "$@" ;;
  -h|--help)  echo "用法: dcc-use [list|current|show|add|edit|remove|test|<name>]" ;;
  *)          cmd_switch "$1" ;;
esac
```
...
修改的是宿主侧 `~/.docker-cc/claude/settings.json`（被卷挂载到容器内 `/root/.claude/settings.json`），下次 `dcc` 启动 claude 时自动生效，无需重启容器。
...
{
  "_mode": "oauth",
  "_comment": "使用 Anthropic 账号订阅；切到此模式后请执行 dcc login"
}
```

> **API Key 与 OAuth 互斥**：Claude Code 一旦读到 `ANTHROPIC_AUTH_TOKEN` 就会绕过 OAuth。`dcc-use` 切到 OAuth 模式时会清空 `env`；切回任意 API key 模式则 OAuth 自动失活（凭据文件保留，不被使用）。
...
├── docker-compose.yml
├── entrypoint.sh
├── VERSION                         # 单行版本号，dcc --version 启动时显示
├── .env.example                    # CLASH_SUB_URL=... + 可选 UI_PORT / GH_PROXY 等
├── .gitignore                      # 包含 .env、~/.docker-cc/ 等敏感路径
├── bin/
│   ├── dcc                          # 宿主 wrapper
│   ├── dcc-use                      # 切供应商（宿主 + 容器复用同一份）
│   └── _dcc-probe-ghproxy           # 探测可用 GH_PROXY 镜像源（install.sh + dcc probe 共享）
...
3. 把整个仓库内容复制到 `~/.docker-cc/repo/`（包含 `Dockerfile`、`entrypoint.sh`、`bin/`、`docker-compose.yml`、`.env.example`、`providers/`），保证 `dcc upgrade` 时 `docker compose build` 有完整的 build context。
...
7. 把 `bin/dcc` 和 `bin/dcc-use` 软链到 `/usr/local/bin/`。
8. 提示用户：用 `dcc up <subscription-url>` 启动，并 `dcc-use edit <name>` 填 token。
...
| `--skip-link` | 跳过把 `dcc` / `dcc-use` 软链到 `/usr/local/bin/`（不需要 sudo） | 隔离测试；不污染系统 PATH |
...
### 首次配置
```bash
git clone <repo> && cd docker-dcc
./install.sh
dcc up "https://your-airport.com/subscription"    # 直接传订阅 URL，自动写入 .env 并启动
$EDITOR ~/.docker-cc/providers/deepseek.json     # 填 LLM 供应商 token
dcc panel                                         # 浏览器打开面板，选个节点
```

> 也可以先 `$EDITOR ~/.docker-cc/repo/.env` 手动填 `CLASH_SUB_URL`，然后 `dcc up`（不带参数）。两种等价。

### 日常使用

```bash
# —— 启停与订阅管理 ——
dcc up                                # URL 已在 .env，直接启动 mihomo
dcc up "https://new-airport.com/sub"  # 换机场：传新 URL，自动覆盖 .env 并重拉
dcc refresh                           # URL 不变，仅重拉一次订阅（节点更新）
dcc down                              # 停服务
dcc logs -f                           # 看 mihomo 日志
dcc panel                             # 浏览器打开 metacubexd 面板（19090）
dcc probe                             # 重新探测 GH_PROXY 镜像源（dcc upgrade 失败时救援）
dcc upgrade                           # 升级镜像内的 claude code 到最新版（rebuild）
...
# —— 管理 LLM 供应商 ——
dcc-use                               # 列出所有，★ 标记当前
dcc-use edit anthropic                # 编辑 install.sh 预置的模板（填入 token）
dcc-use add my-account \              # 新增一个非预置供应商
    --api-key=sk-xxx --base-url=https://api.example.com/anthropic \
    --model=foo-pro --small-model=foo-mini
dcc-use show kimi                     # 查看详情（API Key 脱敏）
dcc-use test                          # curl 测试当前供应商连通性
dcc-use remove my-account             # 删除（带确认）

# —— 切换 LLM 供应商 ——
dcc-use kimi                          # 切到 Kimi（API Key 模式）
dcc                                   # 用 Kimi 跑

# —— 用 Claude 账号订阅（OAuth 模式）——
dcc login                             # 切到 claude-account，然后进入 claude 交互界面
                                     # 在会话内手动敲 /login → 终端显示 URL → 宿主浏览器
                                     # 授权 → 拷贝 token 回粘 → Ctrl+D 退出（凭据已落盘）
dcc                                   # 之后正常使用，走 Claude Pro/Max 配额
dcc logout                            # 同样进交互界面，敲 /logout
dcc-use deepseek                      # 切回 API Key 供应商，OAuth 自动失活
```
...
宿主 ~/.docker-cc/claude/.credentials.json
   │  (卷挂载，dcc login 后由 claude 写入)
   ▼
容器 /root/.claude/.credentials.json ──► claude 走 OAuth 鉴权
```

`dcc-use` 在宿主直接改 `~/.docker-cc/claude/settings.json`，下次 `dcc` 启动新容器即生效。OAuth 凭据文件落在同一目录，自动持久化跨容器复用。
...
~/.docker-cc/claude/settings.json    ← 当前 LLM 供应商配置（dcc-use 写入）
~/.docker-cc/claude/.credentials.json← OAuth 凭据（dcc login 写入）
~/.docker-cc/claude/<其他>           ← Claude Code 会话历史、todo 状态等
~/.docker-cc/providers/*.json        ← 各 LLM 供应商模板（含 token）
```

`cache.db` 是 mihomo 自己维护的状态文件，**下次 `dcc up` 后面板里看到的节点选择和你上次离开时完全一样**，无需重新选。
...
dcc-use anthropic                # 切到该供应商

# 日常：直接 dcc，主对话用 Sonnet，小快自动用 Haiku
dcc
...
| 触发动作 | 影响 |
|---|---|
| `dcc up`（无参，默认） | 复用现有 config.yaml 和 cache.db，秒起 |
| `dcc up <new-url>` | 覆盖 .env，强制重拉订阅；节点选择会重置（新订阅节点 ID 变了） |
| `dcc refresh` | URL 不变，仅重拉订阅；若新订阅保留了同名节点，cache.db 仍能命中你之前的选择 |
| 手动删 `~/.docker-cc/mihomo/cache.db` | 节点选择回到订阅默认，配置不变 |
| 手动删 `~/.docker-cc/mihomo/config.yaml` | 下次 `dcc up` 自动重拉订阅 |

---

## 12. 已知限制与边界

- **正在运行的 `claude` 进程不会感知 `dcc-use` 切换**：必须退出当前会话重新 `dcc` 才生效（与 cc-switch GUI 行为一致）。
- **OAuth 与 API Key 互斥**：`ANTHROPIC_AUTH_TOKEN` 一旦在 env 中存在，Claude Code 会优先使用，OAuth 凭据被忽略。`dcc-use claude-account` / `dcc login` 会清空 env；切回其他供应商即恢复 API Key 模式。
...
- **多并发会话**：可以多个 terminal 同时 `dcc`，每个新建独立 dcc 容器，共享同一个 mihomo。
- **容器内 dcc-use 只读**：providers 目录在 dcc 容器内以 `:ro` 挂载，容器内只能 `list` / `show` / `test` / `<name>` 切换；`add` / `edit` / `remove` 必须从宿主调用（用宿主的 `dcc-use` 软链）。这是安全设计：避免容器内程序意外破坏供应商配置。
...
- **首次冷启动**：mihomo 拉订阅 + 启动 ~3-5 秒；之后 `dcc` 启动 ~1 秒（仅创建 dcc 容器）。
- **不能用 `claude update` 自更新**：Claude Code 装在镜像层，`--rm` 临时容器内的 npm 更新会随容器一起销毁。必须用 `dcc upgrade` 在宿主侧 rebuild 镜像。`dcc update` 已被脚本拦截并给出提示。配置 / 凭据 / 节点状态在卷挂载目录里，与镜像无关，升级不会丢。
- **`dcc` 命令名与系统 C 编译器冲突**：macOS/Linux 的 `dcc` 默认是 C 编译器（链接到 clang/gcc）。本方案的 `dcc` 软链放在 `/usr/local/bin/`，**会遮蔽**部分构建脚本中的 `dcc` 调用（如某些 Makefile）。如有冲突，可通过 `DOCKER_CC_CMD=xc ./install.sh` 重命名（或手动建别名）。绝大多数现代项目用 `gcc`/`clang` 显式指定编译器，不受影响。
- **macOS 自带 Bash 3.2 多字节兼容性**：macOS 因 GPLv3 协议拒绝升级，`/bin/bash` 至今是 2007 年的 3.2.57 版本，在 UTF-8 locale 下处理 `$var紧跟中文字符` 的边界识别有 bug —— 会把中文字符的 UTF-8 首字节"吃"进变量名解析，导致变量展开为空且字符破损。**所有 shell 脚本里 `$var` 后紧跟非 ASCII 字符时必须用 `${var}` 显式分隔**。本方案脚本已统一处理。如自行扩展 `dcc-use` / `dcc` / `entrypoint.sh`，注意遵守此规则。
- **`dcc shell` 等命令需 zsh `hash -r` 后生效**：首次安装后 zsh 可能 cache 了系统 `/usr/bin/dcc`（C 编译器）的路径，导致 `dcc <子命令>` 被解析为 clang。修复：在终端跑 `hash -r`，或新开终端窗口（zsh 重新扫描 PATH）。
...
- 加 `dcc doctor` 诊断命令：检查代理通断、API key 有效性、镜像版本。
...
### 步骤 4：dcc / dcc-use 脚本
**做**：实现 `bin/dcc` 和 `bin/dcc-use`
**验证**：
```bash
shellcheck bin/dcc bin/dcc-use                                        # 期望：无 error
ln -sf "$PWD/bin/dcc" /usr/local/bin/dcc
ln -sf "$PWD/bin/dcc-use" /usr/local/bin/dcc-use

# dcc-use 子命令
dcc-use list                                                         # 期望：列出已知供应商
dcc-use add foo --api-key=K --base-url=https://x.com --model=m
test -f ~/.docker-cc/providers/foo.json && echo OK                  # 期望：OK
dcc-use show foo | grep -v 'K' && echo "脱敏 OK"                      # 期望：脱敏 OK
dcc-use foo                                                          # 期望：已切换到 ...
dcc-use current                                                      # 期望：foo
dcc-use remove foo <<< 'y'

# dcc 透传 + PWD 正确性
cd /tmp && mkdir -p test-cc-pwd && cd test-cc-pwd && touch marker
dcc -p "list files in current dir" | grep marker && echo "PWD OK"    # 期望：PWD OK

# dcc up <url> .env 写入正确（含特殊字符的 URL）
dcc up "https://test.com/sub?token=x&format=clash"
grep '^CLASH_SUB_URL=' ~/.docker-cc/repo/.env                       # 期望：双引号包裹完整 URL

# dcc upgrade 不丢配置
dcc-use foo  # 切到 foo
dcc upgrade
dcc-use current                                                      # 期望：foo（升级未影响配置）
```
...
./install.sh                                                        # 期望：所有步骤打 ✓
which dcc dcc-use                                                     # 期望：/usr/local/bin/dcc{,−use}
...
# 卸载干净
./uninstall.sh
test ! -L /usr/local/bin/dcc && echo "卸载 OK"
```
...
**验证**：
```bash
bats tests/unit/        # 期望：全绿（dcc-use 各子命令 ~20+ 用例）
...
| **M2** 脚本层 | ✅ | bin/dcc + bin/dcc-use 完整实现，所有子命令到位 |
...
| # | bug | 修复 | 状态 |
|---|---|---|---|
| 23 | `mirror.ghproxy.com` SSL_ERROR_SYSCALL，build 中断 | `install.sh` 加 GH_PROXY 自动 probe（5 源轮询） + `dcc probe` 子命令 | ✅ 已修 |
| 24 | macOS Bash 3.2.57 在 UTF-8 locale 下 `$var(中文`...）` 变量展开为空 + 中文首字节丢失 | dcc-use 4 处 `$name` → `${name}`；§12 加兼容性说明 | ✅ 已修 |
| 25 | zsh 缓存 `dcc` 路径为 `/usr/bin/dcc`（C 编译器），新装的 `/usr/local/bin/dcc` 不被识别 | §12 补操作说明：`hash -r` 或新开终端 | ✅ 文档化 |
| 26 | `dcc panel` 进入后 metacubexd 报 ERR_CONNECTION_REFUSED：浏览器端硬编码 `127.0.0.1:9090`，但端口避让设计映射到 19090 | `~/.docker-cc/repo/docker-compose.override.yml` 临时加 9090:9090 双映射 | 🟡 临时 |
| 27 | 镜像 `/etc/mihomo/ui` 被卷挂载遮蔽，mihomo 启动时自己又下载一份 UI（绕路） | UI 应解压到非挂载路径如 `/usr/share/metacubexd/`，entrypoint 改 `external-ui` 指向那里 | ⬜ 待修 |
| 28 | providers/*.json 文件权限默认 644（API key 明文，多用户机器风险）| install.sh + dcc-use add 写入时 `chmod 600`；providers 目录 `chmod 700` | ✅ 已修 |
| 29 | .env 文件权限默认 644（CLASH_SUB_URL 可能内嵌订阅 token）| 所有写入位置 `chmod 600` | ✅ 已修 |
| 30 | install.sh 与 bin/dcc probe 探测逻辑代码重复 | 抽到共享脚本 `bin/_dcc-probe-ghproxy`，两处统一调用 | ✅ 已修 |
...
| 33 | `dcc-use test` 探 `/v1/models`，DeepSeek 等兼容供应商不实现，永远返回 404 | 改成 POST `/v1/messages` 占位 model + 状态码语义化解读；附带 `Authorization: Bearer` 头 | ✅ 已修 |
...
| `dcc up <真实订阅>` 拉订阅 + mihomo 启动 | 需要订阅 URL | ✅ 用户已跑通 |
| `dcc panel` 看到节点列表 + 切节点 | 需面板可访问 | ✅ 用户跑通（用 override） |
| 容器内 `curl -x http://127.0.0.1:7890 anthropic.com` 通 | 节点真实可用 | ✅ 用户跑通 |
| `dcc login` OAuth 登录流程 | 需 Claude 账号 + 浏览器 | ✅ 用户跑通（凭据自动复用） |
...
├── tests/
│   ├── unit/{dcc-use_add,dcc-use_switch,dcc-use_show,dcc-use_current,dcc-use_remove,dcc-use_list,dcc_up,dcc_upgrade}.bats  ✅
...
| **M2** | 脚本层 | 3 d | `dcc` / `dcc-use` 全套子命令 work | |
...
- T1.2 完成 `entrypoint.sh` 的 `dcc 服务分支`（健康检查 mihomo:9090、DCC_PROVIDER 切换、exec）
- T1.3 手动放 `~/.docker-cc/providers/test.json` 验证 claude 调用

**测试任务**：
- TT1.1 `tests/integration/04_first_run.bats`：`dcc up <mock-url>` 30s 内不卡死（HTTP_PROXY 回环回归）
...
**开发任务**（dcc 与 dcc-use 可两人并行）：
- T2.1 `bin/dcc-use`：cmd_list / current / show / add / edit / remove / test / switch（§7.2）
- T2.2 `bin/dcc`：up / down / panel / shell / login / logout / refresh / upgrade / update / 默认（§7.1）
- T2.3 容器/宿主路径自动适配（dcc-use 头部 `/.dockerenv` 探测）
- T2.4 防御逻辑：HOST_PWD 保留、source .env 安全提取、拒绝 `claude update`

**测试任务**（TDD 与开发并行）：
- TT2.1 `tests/fixtures/helpers.bash`（[testing.md](testing.md) §1.1）
- TT2.2 全部 `tests/unit/dddcc-use_*.bats`（add / switch / show / current / remove / list）
- TT2.3 `tests/unit/dcc_up.bats`（含特殊字符 URL 回归 #19 #20）
...
**风险**：
- 🔴 dcc 脚本 PWD 处理 bug（已修过一次）→ bats 守门
- 🔴 dcc-use 脱敏漏 token → bats 断言无完整 token
- 🟡 jq 跨平台行为差异 → 用纯 jq 表达式，不依赖外部管道

**依赖**：M1 done（dcc 脚本调 docker compose 要真能起容器）。

**并行**：T2.1 / T2.2 接口约定一致后两人并行；测试 TDD 与开发并行。

**交付物**：`dcc -p "hi"`、`dcc-use kimi`、`dcc-use add foo --api-key=...` 全部 work。
...
- TT3.2 `tests/integration/07_upgrade_persistence.bats`（dcc upgrade 不丢配置）
...
**交付物**：新机器 `git clone && ./install.sh && dcc up <url> && dcc -p "hi"` 一气呵成。
...
| R2 | Claude Code 大版本不兼容 | npm 包重命名环境变量 | 锁 npm 版本 + dcc-use 加版本探测 warn |
...
│   ├── dcc
│   ├── dcc-use
│   └── _dcc-probe-ghproxy           # 共享探测脚本，install.sh 和 dcc probe 调用
├── providers/
...
| 版本 | 预计 | 内容 |
|---|---|---|
| **v0.2.0** | +2 周 | `dcc doctor` 诊断命令、starship prompt 集成、dcc-use TUI 选择器 |
