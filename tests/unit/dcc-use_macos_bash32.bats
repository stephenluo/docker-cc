...
@test "dcc-use switch: 输出含完整变量值 + 完整中文括号（不漏字节）" {
...
@test "dcc-use add OAuth: 输出含完整变量值 + 完整中文括号" {
  run dcc-use add my-oauth --mode=oauth
  [ "$status" -eq 0 ]
  [[ "$output" =~ "my-oauth" ]]
  [[ "$output" =~ "（" ]]
  [[ "$output" =~ "）" ]]
...
@test "dcc-use add 重名: 报错信息含完整供应商名" {
  echo '{"ANTHROPIC_AUTH_TOKEN":"K","ANTHROPIC_BASE_URL":"U"}' > "$PROVIDERS_DIR/foo.json"
  run dcc-use add foo --api-key=K --base-url=U
  [ "$status" -ne 0 ]
